# Deploying airlytic-webodm to 3dm.airlytic.co

Operational playbook for shipping changes from this fork to the production WebODM instance at `https://3dm.airlytic.co` (single EC2 host, SSL via certbot, behind no proxy).
...
./webodm.sh start --ssl --hostname 3dm.airlytic.co --detached
```
...
> ```bash
> ./webodm.sh start \
>   --hostname 3dm.airlytic.co \
>   --ssl-key /etc/letsencrypt/live/3dm.airlytic.co/privkey.pem \
>   --ssl-cert /etc/letsencrypt/live/3dm.airlytic.co/fullchain.pem \
>   --port 443 \
...
```bash
curl -ks https://3dm.airlytic.co/api/projects/?page=1 -o /dev/null -w "%{http_code}\n"
# expect: 200
```
...
This instance uses Let's Encrypt via `webodm.sh --ssl`, which runs `nginx/letsencrypt-autogen.sh` to fetch a cert with `certbot certonly --standalone --http-01-port 8080`. The cert is stored at `./letsencrypt/live/3dm.airlytic.co/` and symlinked into `./ssl/key.pem` and `./ssl/cert.pem`.

**First-time setup or after a server rebuild** — when certs are missing entirely:

```bash
./webodm.sh restart --ssl --hostname 3dm.airlytic.co --detached
```

`restart` does `down` + `start`, which is required on first setup so certbot can bind port 80 for the HTTP-01 challenge. Ports 80 and 443 must be open in the AWS Security Group.
...
### `https://3dm.airlytic.co` returns ERR_CONNECTION_REFUSED after a restart
...
If certbot has none, re-run the first-time setup path above (`./webodm.sh restart --ssl --hostname 3dm.airlytic.co --detached`) — autogen will fetch a fresh cert.
...
- [ ] Backup the DB if the PR touches models:
  ...
  ```
- [ ] Confirm AWS Security Group allows 80 + 443 inbound
